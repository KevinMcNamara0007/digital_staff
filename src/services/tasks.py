...
async def process_agent_tasks(tasks, user_prompt, file_list, repo_dir, new_branch_name, all_code):
    ...
    all_agent_responses = "".join([f"{{Agent: {i}, Response: {response}}}" for i, response in enumerate(tasks_responses)])
...
async def run_python_tests(repo_dir, present_venv_name=None, tries=3):
    ...
    if not os.path.exists(requirements_path):
        with open(requirements_path, 'w'):
            pass
    await cmd_popen(repo_dir, f"{path_to_python_exec} -m pip install -r requirements.txt pytest pytest-cov httpx", shelled=True)
...
async def run_pytest_and_analyze(repo_dir, path_to_python_exec):
    stdout, stderr = await cmd_popen(repo_dir, f"{path_to_python_exec} -m pytest -p no:cacheprovider --no-cov", sterr=True, shelled=True)
    output = stdout + stderr
    missing_packages, code_errors, syntax_errors, general_errors = parse_pytest_output(output)
    known_packages = [pkg for pkg in missing_packages if find_package_from_import(pkg)]
    unknown_packages = [pkg for pkg in missing_packages if not find_package_from_import(pkg)]
    return {
...
def parse_pytest_output(output):
    ...
    code_errors = [
        {'file': match.group(1), 'line': match.group(2), 'function': match.group(3)}
        for match in code_error_pattern.finditer(output)
    ]
    syntax_errors = [
        {'file': match.group(1), 'line': match.group(2), 'message': match.group(3)}
        for match in code_error_pattern.finditer(output)
    ]
    general_errors = [
        {'error': general_error_match.group(1), 'traceback': (traceback_match := traceback_pattern.search(output)).group(1).strip() if traceback_match else None}
...
async def failure_repair(missing_packages, code_errors, syntax_errors, general_errors, repo_dir, venv_name, tries=3):
    ...
    if syntax_errors:
        await handle_code_errors(syntax_errors)
        return await retry_tests(repo_dir, venv_name, tries - 1)
    if general_errors:
...
async def handle_code_errors(errors):
    for error in errors:
        file_path, line_number, error_message = error['file'], int(error['line']), f"Error in function {error['function']}"
